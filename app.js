...
(function () {
    ...
    API.dashboardStats({ scope }).then((res) => {
      ...
      const enterpriseDesc = scope === 'enterprise' ? `
        <div class="card desc-panel desc-panel-sm">
          ...
          <ul>
            <li><b>1·作业数量：</b>待审核，待开始，进行中，已完成这几个状态的作业总计；</li>
        ...
        <div style="display:flex;gap:16px">
          <div class="side-desc-panel mini-side-panel">
            ...
            <ul>
              ...
              <li><b>6·当安管员返回到首页或重新进入小程序，只要有超时未核查的作业，就一直弹核查提醒弹框。样式为核查提醒弹框，内容填写【当前有xx条作业已超时，请尽快完成现场核查】，然后首页展示所有超时的作业名称。</li>
            </ul>
          </div>
          <!-- 核查提醒弹框展示 -->
  ...
  function renderMiniVerifyCards(verifies, workId, showAuditBtn = false) {
    ...
    return verifies.map((v, i) => {
      ...
      return `<div class="mini-rec-card ${isOpen ? 'open' : ''}" data-idx="${i}">
        ...
        <div class="mini-rec-body">
          <div class="mini-li-row"><span class="mini-li-label">定位记录</span><span class="mini-li-val">${esc(v.location || '杭州市钱塘区')}</span></div>
  ...
  function showMiniAiReview(photoCount, videoCount, seedBase) {
    ...
    const descHtml = `<div class="ai-review-desc">
      ...
      <ul>
        <li>点击照片打开弹框，展示该照片或视频具体的检测记录信息、检测依据、法律法规、整改措施，具体样式参照总台--智慧应急--设备检测或在线检测的检测记录详情，保持一致即可。</li>
      </ul>
    </div>`;
...
<div class="mini-li-row"><span class="mini-li-label">作业区域</span><span class="mini-li-val">${esc(w.areaName || '')}</span></div>
        ...
        <div class="side-desc-panel mini-side-panel">
          ...
          <ul>
            ...
            <li><b>4·审核记录：</b>现场图片和现场视频来源为作业人员上传的；检测记录展示查看详情按钮；拒绝原因为安管员审核拒绝时手动输入的内容，待审核状态审核人员，时间，原因都是横杠，若ai审核异常自动拒绝时，审核人员横杠，时间为ai审核时间，拒绝原因固定为【审核自动拒绝】</li>
            <li><b>5·现场核查记录：</b>作业区域为安管员现场核查时手动输入的具体地址，检测记录展示查看详情按钮。上传核查内容的所有照片仅支持实时拍摄，不允许相册上传。</li>
            ...
            <li><b>9·现场核查的功能主要发生在进行中的任务，但待审核也支持操作，如果待审核的作业里上传了现场核查内容，则先展示现场核查的审核，审核之后再进行作业审核，两个审核按钮相互独立，不会相互响应，但要先审核现场核查再审核作业。</li>
          </ul>
        </div>
      </div>
    `;
...
<div class="mini-li-row"><span class="mini-li-label">作业区域</span><span class="mini-li-val">${esc(w.areaName || '')}</span></div>
        ...
        <div class="side-desc-panel mini-side-panel">
          ...
          <ul>
            ...
            <li><b>4·已完成的作业详情里会多一个模块【完成记录】：</b>该模块展示安管员和作业负责人上传的完结证明，上传了即展示，未上传就显示横杠，目前只有作业人员和安管员，没有监护人确认记录，但不排除以后会加上。</li>
            <li><b>5·完成记录：</b>作业完成记录是作业负责人上传的完成证明，状态固定为已完成，完成时间是负责人提交完结证明时间；安管员确认记录的状态固定为已确认，确认时间为安管员提交完结证明时间。</li>
          </ul>
        </div>
      </div>
    `;
...
<div class="mini-li-row"><span class="mini-li-label">作业区域</span><span class="mini-li-val">${esc(w.areaName || '')}</span></div>
        ...
        <div class="side-desc-panel mini-side-panel">
          ...
          <ul>
            ...
            <li><b>2·作业现场：</b>展示提交作业时上传的照片和视频。如果该条作业是由已拒绝再次发起的则作业现场下面可能会存在审核记录和现场核查记录，有就展示，无则隐藏。</li>
          </ul>
        </div>
      </div>
    `;
...
<div class="mini-li-row"><span class="mini-li-label">作业区域</span><span class="mini-li-val">${esc(w.areaName || '')}</span></div>
        ...
        <div class="side-desc-panel mini-side-panel">
          ...
          <ul>
            ...
            <li><b>4·已完成的作业列表会有上传完结证明一次性操作按钮，只有负责人才能点击，上传照片即可，姓名回填当前登录的负责人姓名，详情页底部只有已完成的作业会多一个【完成记录】模块，展示负责人和安管员的确认完结信息。</li>
          </ul>
        </div>
      </div>
    `;
      ...
      <div class="mini-office-wrap">
        ...
        <div class="side-desc-panel mini-side-panel">
          ...
          <ul>
            ...
            <li><b>4·作业区域：</b>由二维码带过来的该企业被G端绑定的所有区域信息，如果二维码不带区域信息则这里展示一个横杠；如果下拉框只有一条数据则自动填充；多条数据则点击下拉框自己选。</li>
            <li><b>5·门店名称：</b>下拉框，数据为该企业下的所有门店数据，如果企业后台没有配置门店，则这里直接展示横杠，且施工地址也不会回填门店地址。</li>
            ...
            <li><b>10·作业开始时间/作业结束时间：</b>仅选择时和分（如09:00、18:00），需在作业周期范围内。结束时间要大于开始时间，本期不考虑跨夜情况。</li>
            ...
            <li><b>15·负责人：</b>作业信息里填写了施工负责人，这个人必须填写在施工人列表，且自动标记为负责人（人员信息要以身份三要素判定唯一性），且负责人这条记录不支持删除只能编辑。</li>
            <li><b>16·作业现场：</b>支持仅照片或仅视频或照片和视频，属于必填但至少有一项，都没有则无法提交。注意作业现场只支持手机实时拍摄，不允许打开相册上传。</li>
        ...
        <div class="mini-req-panels">
          ...
          <div class="mini-req-panel">
            ...
            <ol class="mini-req-list">
              ...
              <li>2·政府端和企业端作业管理的作业列表，作业与区域放在最前列且固定，并将所有状态列放前面</li>
              <li>3·二维码名字更改，且加上海报和去现场扫码的提醒与上传作业证书的提醒</li>
              <li>4·添加施工人，需要查询库里该人员的作业证书自动回填，没有证书则需要先上传（总台证书有问题，暂时无法联动，暂时先不做）</li>
              <li>5·G端作业区域增加主管部门</li>
              <li>6·政府端作业区域添加的账号可以登录安管员小程序，拥有审核和上传现场核查的功能（不需要上传完结证明功能，不受核查规则影响也没有核查提醒，该类账号审核和现场核查审核，在审核记录里审核人员显示部门名称-姓名；核查记录模块的表头名字也显示部门名称-姓名）</li>
            </ol>
            <div class="mini-req-meta">
      ...
      API.listAreas({ status: state.status }).then((res) => {
        ...
        const desc = h(`<div class="card desc-panel desc-panel-sm">
          ...
          <ul>
            ...
            <li><b>2·作业数量：</b>待审核，待开始，进行中，已完成这几个状态的作业总计；</li>
            <li><b>3·主管部门统计数量，点击展示主管部门，姓名，手机号，一条信息展示一行，该手机号可以登录安管员小程序，可查看该区域下所有企业的作业信息，并在待审核，待开始，进行中的作业随时上传现场核查，不受核查规则约束，也不会因为核查规则导致弹框预警。【本期只做新增和展示，不做功能交互】</b></li>
          </ul>
        </div>`);
        ...
        const descDisable = h(`<div class="card desc-panel desc-panel-sm">
          ...
          <ul>
            <li><b>禁用/删除：</b>操作时需判断当前区域下绑定的企业是否有待审核，待开始，进行中的作业，如果有，则不能禁止或删除并给出提示。</li>
          </ul>
        </div>`);
      ...
      <div class="modal-with-desc">
        ...
        <div class="modal-desc">
          ...
          <ul>
            ...
            <li>主管部门/姓名/手机号这三个输入框初始状态都是非必填，但如果填写了任何一条内容则三个都必须全部填写，即要么三个都不填要么全填（因为如果填写了手机号其余两个未填，会导致安管员小程序上展示异常）</li>
          </ul>
        </div>
      </div>`;
      ...
      API.listEnterprises({ areaId: state.areaId }).then((res) => {
        ...
        const desc = h(`<div class="card desc-panel desc-panel-sm">
          ...
          <ul>
            ...
            <li><b>3·解绑：</b>解绑时需判断当前企业是否有待审核，待开始，进行中的作业，若有则无法解绑并给出提示。</li>
          </ul>
        </div>`);
        slot.parentElement.appendChild(desc);
        // 新增：字段说明
        ...
        if (opt.scope === 'enterprise') {
          const desc = h(`<div class="card desc-panel desc-panel-sm">
            ...
            <ul>
              ...
              <li><b>3·作业人员照片/监护人照片/现场核查状态：</b>取该作业现场核查记录里最新一条的状态，如果最新一条未审核即没有状态标识，则展示横杠，照片上传了则显示已上传反之未上传。如果作业里没有现场核查记录，则这三个字段显示横杠</li>
