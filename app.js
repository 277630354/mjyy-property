...
<div class="mini-li-row"><span class="mini-li-label">作业区域</span><span class="mini-li-val">${esc(w.areaName || '')}</span></div>
        ...
        <div class="side-desc-panel mini-side-panel">
          ...
          <ul>
            ...
            <li><b>4·审核记录：</b>现场图片和现场视频来源为作业人员上传的；检测记录展示查看详情按钮；拒绝原因展示ai拒绝原因，如果安管员输入了拒绝原因，则展示拒绝时填写的内容覆盖掉ai结果。</li>
            <li><b>5·现场核查记录：</b>作业区域为安管员现场核查时手动输入的具体地址，检测记录展示查看详情按钮，不通过原因是ai结果，因为安管员上传现场照片视频后，交由ai评判，没有后续的人为操作。</li>
...
<div class="mini-li-row"><span class="mini-li-label">作业区域</span><span class="mini-li-val">${esc(w.areaName || '')}</span></div>
  ...
  function modalWorkDetail(w, scope) {
    ...
    const aiItems = scope === 'enterprise' ? `
        <li><b>3·审核记录：</b>小程序上传过来的数据，其中现场图片和现场视频调用智慧应急，先由AI判断是否存在违规，如果有直接判定为审核不通过，折叠行的状态标记和审核状态都自动显示为已拒绝，且自动回填AI返回的拒绝原因在【不通过原因】字段；拒绝原因展示审核拒绝时填写的内容，若未填写拒绝原因或因为ai检测被判定的不通过则展示ai结果。</li>
        <li><b>4·现场核查记录：</b>同审核记录，AI识别图片视频，返回不通过原因，状态标记为【异常】，目前仅展示异常不做后续处理，后续可能会增加异常告警之类的功能。</li>` : '';
    const sidePanel = h(`<div class="side-desc-panel">
